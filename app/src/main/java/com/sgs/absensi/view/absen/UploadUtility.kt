...
class UploadUtility(activity: Activity) {
    ...
    var serverURL: String = "http://103.152.119.231/Absensi_apps_web/api_upload_absen.php"
    var serverUploadDirectoryPath: String = "http://103.152.119.231/Absensi_apps_web/assets/foto_absen"
...
//                    showToast("File uploaded successfully at $serverUploadDirectoryPath$fileName")
                    showToast("File sukses diupload")
                } else {
                    Log.e("File upload", "failed")
                    showToast("File gagal diupload")
                }
            } catch (ex: Exception) {
                ex.printStackTrace()
                Log.e("File upload", "failed")
